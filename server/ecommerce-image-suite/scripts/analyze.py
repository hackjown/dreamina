...
```json
{
  ...
  "product_type": "食品/茶叶 | 家居用品 | 3C数码 | 美妆 | 服装 | 其他",
  "garment_position": "non-apparel（非服装统一填 non-apparel）",
  ...
  "product_style": "商品风格（如：中式典雅 / 现代简约 / 工业风 / 运动休闲）",
  "color": "精确英文色值描述（如 china red、gold、matte black）",
  "material": "主要材质（如：金属/铁罐、陶瓷、塑料、纯棉）",
  "style": "外观特征描述（如：圆柱形、方形、修身型等）",
  "print_design": "包装图案/印花设计描述",
...
重要指令：
1. 请先进行 OCR 扫描，识别图片中出现的任何文字。
2. 重点观察商品本身，忽略背景中的杂乱文字。
3. 若商品包装上有品牌名，请务必在 product_name_zh 中体现。
4. 如果无法确定具体品牌，请根据视觉特征描述商品（例如：“红色圆柱形金属茶叶罐”），不要返回“未识别”或“无法识别”。
5. 必须输出完整的 JSON 字符串，包含所有字段。"""
...
def extract_json(text: str) -> dict:
    ...
    raw = text.strip()
    # 打印原始文本长度以便调试
    print(f"   [debug] 原始响应长度: {len(raw)} 字符", file=sys.stderr)
    
    # 尝试寻找 JSON 代码块
    if "```json" in raw:
        try:
            content = raw.split("```json", 1)[1].split("```", 1)[0].strip()
    ...
    if "```" in raw:
        try:
            content = raw.split("```", 1)[1].split("```", 1)[0].strip()
            # 如果内容以 json 开头（没有在 split 时被切掉的情况）
    ...
    if start != -1 and end > start:
        try:
            ...
            raise ValueError(f"JSON 解析失败: {e}\n片段: {raw[start:end][:200]}...")
            
    raise ValueError(f"无法从输出中提取 JSON。输出内容预览: {raw[:200]}...")
...
def _is_private_base_url(base_url: str) -> bool:
    """局域网/本机 API 不走系统代理，避免 OpenAI SDK 被代理卡住。"""
    try:
...
def _extract_completion_text(completion) -> str:
    """兼容标准 OpenAI 对象、dict，以及少数兼容接口直接返回字符串的情况。"""
    if isinstance(completion, str):
        text = completion.strip()
        if not text:
            return ""
        try:
            return _extract_completion_text(json.loads(text))
        except Exception:
            return text

    if hasattr(completion, "model_dump"):
        try:
            return _extract_completion_text(completion.model_dump())
        except Exception:
            pass
    ...
    try:
        ...
        raise ValueError(f"无法读取模型响应内容，响应类型: {type(completion).__name__}") from exc
...
def analyze(image_paths_or_urls: list, provider: dict, api_key: str,
    ...
    try:
        ...
        return {"error": str(e), "raw": repr(completion)[:1000]}
