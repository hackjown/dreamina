...
def build_prompt(type_id: str, desc: str, selling_points: list,
    ...
    PRODUCT_REF_LOCK = (
        ...
        "Keep EXACT same: overall silhouette, shape, surface texture, material finish, color, proportions, markings, pattern placement and distinctive details. "
        "DO NOT change: the product structure, color scheme, material appearance, brand mark, pattern, logo or key visual features. "
        "You may ONLY change: background scene, camera angle, lighting, model pose. "
        "The product must look IDENTICAL to the reference image - same product, same material, same colors and same design details."
    )
    ...
    if lang == "zh":
        TEXT_RENDER = (
            ...
            "Use premium brand-catalog typography: refined Chinese title lettering with medium weight, "
            "clean modern Chinese sans-serif body text with regular or medium weight. "
            "Never render any font name, software name, layout instruction, or typography label as visible text. "
            "Text must be perfectly sharp, highly legible, excellent hierarchy, proper kerning, and generous line spacing. "
            "Use adaptive premium colors: charcoal instead of pure black on light backgrounds, ivory/soft white on dark or photo backgrounds, "
            "and one restrained accent color sampled from the product or a subtle metallic/brand accent. "
            "Avoid harsh pure black, neon colors, over-saturated red/yellow badges, cheap promotional styling, and oversized poster-like type. "
            "Text must stay in clear negative space, never cover important product details, and occupy no more than 25% of the image. "
            "If text is placed over a photo, add a subtle local dark/light gradient or soft shadow only for readability. "
        ...
        TEXT_RENDER = (
            ...
            "Use premium brand-catalog typography: refined medium-weight display title and clean modern sans-serif body text. "
            "Never render any font name, software name, layout instruction, or typography label as visible text. "
            "Text must be perfectly sharp, highly legible, excellent hierarchy, proper kerning, and generous line spacing. "
            "Use adaptive premium colors: charcoal instead of pure black on light backgrounds, ivory/soft white on dark or photo backgrounds, "
            "and one restrained accent color sampled from the product or a subtle metallic/brand accent. "
    ...
    if kf_style == "icon_list":  # 信息图标列表
        key_features_prompt = (
            ...
            f"Right: refined medium-weight heading \"{kf_heading}\", "
        ...
        key_features_prompt = (
            f"{desc}, ultra-minimalist product photography on deep charcoal background. "
        ...
        key_features_prompt = (
            ...
            f"Subtle energetic radial accent behind product, restrained warm accent color, not neon. "
            f"Three circular badge labels in a tasteful product-matched accent color, medium-bold type with slight angle: "
            f"\"{kf_labels[0]}!\", \"{kf_labels[1]}!\", \"{kf_labels[2]}!\". "
            f"Modern energetic commercial style without cheap promotional clutter. "
            f"{TEXT_RENDER}{ref_tail} {QUALITY}"
        )
    ...
    if template_set == 2:    # 生活杂志：咖啡馆窗前
        selling_pt_prompt = (
            (
                ...
                f"{desc} casually arranged in lifestyle context. Refined medium-weight heading \"{sp_heading}\" upper left, "
                f"\"{sp_sub1}\", \"{sp_sub2}\". Kinfolk lifestyle mood. "
                f"{TEXT_RENDER}{ref_tail} {QUALITY}"
            ) if is_apparel else (
                f"Bright café window seat, morning natural light, warm wooden surface. "
                f"{desc} placed as a lifestyle product hero. Refined medium-weight heading \"{sp_heading}\" upper left, "
        ...
        selling_pt_prompt = (
            (
                ...
                f"Single refined medium-weight heading \"{sp_heading}\" top, \"{sp_sub1}\", \"{sp_sub2}\" below. "
                ...
                f"product as hero object. Single refined medium-weight heading \"{sp_heading}\" top, "
        ...
        selling_pt_prompt = (
            (
                f"Vibrant outdoor urban street, rich but controlled colors, dynamic energy. "
                f"{desc} featured prominently. Refined medium-weight heading \"{sp_heading}\", "
                f"\"{sp_sub1}\", \"{sp_sub2}\". Pop art energy. {TEXT_RENDER}{ref_tail} {QUALITY}"
            ) if is_apparel else (
                f"Vibrant high-energy commercial setting, rich but controlled colors. "
                f"{desc} showcased confidently as hero product. Refined medium-weight heading \"{sp_heading}\", "
                f"\"{sp_sub1}\", \"{sp_sub2}\". Pop art energy. {TEXT_RENDER}{ref_tail} {QUALITY}"
            )
        )
        ...
        selling_pt_prompt = (
            (
                ...
                f"Refined heading \"{sp_heading}\" in restrained metallic accent, \"{sp_sub1}\", \"{sp_sub2}\" in soft white. "
                f"{TEXT_RENDER}{ref_tail} {QUALITY}"
            ) if is_apparel else (
        ...
        selling_pt_prompt = (
            (
                ...
                f"Refined medium-weight heading \"{sp_heading}\" upper left, two lines: \"{sp_sub1}\", \"{sp_sub2}\". "
    ...
    if template_set == 2:    # 生活杂志：木纹/大理石桌面
        material_prompt = (
            (
                ...
                f"Refined medium-weight heading \"{mat_heading}\" upper right, \"{mat_sub1}\" mid, \"{mat_sub2}\" lower. "
                f"{TEXT_RENDER}{ref_tail} {QUALITY}"
            ) if is_apparel else (
        ...
        material_prompt = (
            (
                ...
                f"Single refined medium-weight heading \"{mat_heading}\" right, \"{mat_sub1}\", \"{mat_sub2}\". "
        ...
        material_prompt = (
            (
                ...
                f"rich but controlled colors, close-up angled shot. "
                f"Refined medium-weight heading \"{mat_heading}\" corner, \"{mat_sub1}\", \"{mat_sub2}\" in tasteful accent color. "
                f"{TEXT_RENDER}{ref_tail} {QUALITY}"
            ) if is_apparel else (
                f"{desc} showcased at a dynamic dramatic angle, confident close-up highlighting surface quality, "
                f"rich but controlled colors, energetic composition. "
                f"Refined medium-weight heading \"{mat_heading}\" corner, \"{mat_sub1}\", \"{mat_sub2}\" in tasteful accent color. "
                f"{TEXT_RENDER}{ref_tail} {QUALITY}"
            )
        )
        ...
        material_prompt = (
            (
                ...
                f"Refined heading \"{mat_heading}\" right in restrained metallic accent, \"{mat_sub1}\", \"{mat_sub2}\" in soft white. "
                f"{TEXT_RENDER}{ref_tail} {QUALITY}"
            ) if is_apparel else (
    ...
    if template_set == 2:    # 生活杂志：自然光氛围感
        lifestyle_prompt = (
            (
                ...
                f"Refined medium-weight heading \"{ls_heading}\" upper left, \"{ls_sub1}\" and \"{ls_sub2}\". "
        ...
        lifestyle_prompt = (
            (
                f"Vibrant dynamic scene with rich but controlled colors, energetic atmosphere. "
                ...
                f"{desc} featured confidently as the visual focus. "
                f"Refined medium-weight heading \"{ls_heading}\" upper left, \"{ls_sub1}\" and \"{ls_sub2}\". "
                f"{TEXT_RENDER} {QUALITY}{lock_tail}{ref_tail}"
            )
        )
        ...
        lifestyle_prompt = (
            (
                ...
                f"Refined heading \"{ls_heading}\" upper left in restrained metallic accent, \"{ls_sub1}\" and \"{ls_sub2}\" in soft white. "
        ...
        lifestyle_prompt = (
            (
                ...
                f"Refined soft-white heading \"{ls_heading}\" upper left with subtle readability treatment, \"{ls_sub1}\" and \"{ls_sub2}\" lower left. "
    ...
    if template_set == 2:    # 生活杂志：三格日记卡片
        ...
        multi_scene_prompt = (
            (
                f"[Minimal 2-panel split] Clean thin dividing line at center. "
        ...
        multi_scene_prompt = (
            (
                f"[Dynamic diagonal collage] 45-degree clean split. "
                ...
                f"Centered refined heading \"{ms_heading}\". Bottom: \"{ms_left}\" left, \"{ms_right}\" right. "
                f"High-energy modern composition, rich but controlled tones. {_ms_consistency} "
                f"{TEXT_RENDER} {QUALITY}{lock_tail}{ref_tail}{_sp_model_lock}"
            ) if is_apparel else (
                f"[Dynamic diagonal collage] Clean diagonal split. "
                f"Upper-left: {_ms_scene1}, {desc} vibrant. Lower-right: {_ms_scene2}, {desc} dynamic. "
                f"Refined heading \"{ms_heading}\". {_ms_s1_label} / {_ms_s2_label}. "
        ...
        multi_scene_prompt = (
            (
                ...
                f"Heading \"{ms_heading}\" in restrained metallic accent. Bottom: \"{ms_left}\" left, \"{ms_right}\" right. "
                ...
                f"Heading \"{ms_heading}\" in restrained metallic accent. CRITICAL: same {desc}. "
                f"{TEXT_RENDER} {QUALITY}{lock_tail}{ref_tail}"
            )
        )
    ...
    # 多场景图不让模型排字：中文标题/标签由后端用真实字体叠加，避免模型画错中文字。
    multi_scene_prompt = (
        (
            f"[NO-TEXT commercial 3-panel photography collage] Create one polished e-commerce image with three equal vertical photo panels. "
            f"The SAME {desc} appears in all panels, worn by a consistent relatable {_model_subject}; identical design, color, texture and proportions. "
            f"Left panel visually shows a real-life scene like {_ms_scene1}; center panel visually shows {_ms_scene2}; right panel visually shows {_ms_scene3}. "
            f"Use photography only inside the panels: natural lighting, clean composition, thin white dividers, no graphic overlays. "
            f"Reserve a clean blank light top band and a clean blank light bottom band for later typography overlay. "
            f"ABSOLUTELY NO visible text, letters, numbers, captions, headings, labels, scene names, prompt fragments, signage, logos-as-text, or watermarks anywhere. "
            f"Scene descriptions are visual guidance only and must never appear as text. "
            f"{_ms_consistency} {QUALITY}{lock_tail}{ref_tail}{_sp_model_lock}"
        ) if is_apparel else (
            f"[NO-TEXT commercial 3-panel photography collage] Create one polished e-commerce image with three equal vertical photo panels. "
            f"The SAME {desc} appears in all panels with identical design, color, texture and proportions. "
            f"Left panel visually shows a real-life scene like {_ms_scene1}; center panel visually shows {_ms_scene2}; right panel visually shows {_ms_scene3}. "
            f"Use photography only inside the panels: natural lighting, clean composition, thin white dividers, no graphic overlays. "
            f"Reserve a clean blank light top band and a clean blank light bottom band for later typography overlay. "
            f"ABSOLUTELY NO visible text, letters, numbers, captions, headings, labels, scene names, prompt fragments, signage, logos-as-text, or watermarks anywhere. "
            f"Scene descriptions are visual guidance only and must never appear as text. "
            f"CRITICAL: same {desc} in ALL panels — identical design, color, proportions. {QUALITY}{lock_tail}{ref_tail}"
        )
    )
    ...
    ecommerce_detail_prompt = (
        ...
        f"Main heading \"{_ecd_title}\" in refined medium-weight brand typography, sub-heading \"{_ecd_subtitle}\" below. "
        f"\n"
        ...
        f"3. Six Core Feature Cards (2×3 grid, each: icon + clear medium-weight label + short description): "
    ...
    prompts = {
        "white_bg": (
            f"Isolated product-only e-commerce main image of {desc}. "
            "Pure white seamless background (#FFFFFF), product alone centered, clean studio lighting with a very soft natural shadow. "
            "CRITICAL: remove and forbid every prop or scene element from the reference or description: no cloth, no napkin, no towel, no table, no wooden surface, no plate, no coaster, no plant, no book, no packaging, no hand, no decoration, no lifestyle background. "
            "Only the product itself may appear. Ignore any words in the description about wooden table, natural lighting, lifestyle scene, café, bedroom, home, office, fabric, towel or cloth. "
            f"{white_bg_composition}, product occupies 85% of frame. No text.{ref_tail} {QUALITY}"
        ),
...
DEFAULT_NEGATIVE_PROMPT = (
    ...
    "font name rendered as text, oversized headline, harsh pure black text, neon text, "
    "cheap red/yellow sale badge, cluttered layout, text covering product, cheap look, cartoon"
)
...
def _prepare_openai_edit_image(path_or_url: str, index: int) -> tuple[str, bytes, str]:
    ...
    try:
        ...
        raw = base64.b64decode(_fix_base64_padding(b64_data))
        ext = "png" if mime == "image/png" else mime.split("/")[-1]
        return f"image{index}.{ext}", raw, mime
    finally:
        if temp_path:
            try:
                os.remove(temp_path)
...
def _decode_openai_image_data(data: dict) -> bytes:
    if "b64_json" in data:
        b64_value = data["b64_json"]
    ...
    raise RuntimeError(f"OpenAI 图片响应缺少 url/b64_json 字段: {str(data)[:500]}")


def _request_openai_generation(url: str, key: str, model: str, prompt: str, timeout: int,
                               request_size: str = "1024x1024") -> bytes:
    """优先请求 b64_json，避免 OpenAI 兼容代理生成成功但取不到 URL。"""
    last_error: Exception | None = None
    size_candidates = [request_size or "1024x1024"]
    if size_candidates[0] != "1024x1024":
        size_candidates.append("1024x1024")
    for size in size_candidates:
        for response_format in ("b64_json", "url"):
            try:
                ...
                resp.raise_for_status()
                return _decode_openai_image_data(resp.json()["data"][0])
            except requests.HTTPError as e:
                last_error = e
                status_code = getattr(e.response, "status_code", None)
                # 部分兼容服务不支持 b64_json，此时回退到旧的 URL 格式。
                if response_format == "b64_json" and status_code in (400, 415, 422):
                    continue
                # 部分模型不支持竖版/高分尺寸，此时回退到 1024x1024。
                if size != "1024x1024" and status_code in (400, 415, 422):
                    break
                raise

    if last_error:
        raise last_error
    raise RuntimeError("OpenAI 图片生成失败：未收到有效响应")
...
def generate_openai(key: str, prompt: str, base_url: str = "", model: str = "",
    ...
    if reference_image:
        ...
        if not is_gpt_image and model.startswith("dall-e"):
            ...
            return _request_openai_generation(
                url, key, model, prompt, _kw.get('request_timeout', 360),
                _kw.get("request_size", "1024x1024")
            )
        ...
        for i, ref_path in enumerate(ref_images):
            filename, raw, mime = _prepare_openai_edit_image(ref_path, i)
            # 使用 image[] 作为字段名（该兼容接口要求此字段名）
        ...
        data = {
            ...
            # 当前兼容接口的 /images/edits 只接受 1024x1024；高分/竖版尺寸仅用于纯文生图 generations。
            "size": "1024x1024",
        }
        ...
        try:
            ...
            status_code = getattr(e.response, "status_code", None)
            if data.get("size") != "1024x1024" and status_code in (400, 415, 422):
                data["size"] = "1024x1024"
                try:
                    resp = requests.post(
                        url,
                    ...
                    e = retry_error
                    status_code = getattr(e.response, "status_code", None)
            if status_code in (400, 404, 405, 415, 422, 500, 501):
                fallback_model = model
                if fallback_model.endswith("-edit"):
                    fallback_model = fallback_model[:-5]
                print(
                    f"    ⚠️  OpenAI 兼容接口的 /images/edits 不可用（HTTP {status_code}），"
                    f"将回退为纯文生图（model={fallback_model}）；如需锁定商品外观，请换支持图生图/参考图的模型。",
                    file=sys.stderr,
                )
                return generate_openai(key, prompt, base_url, fallback_model, reference_image="", **_kw)
            raise
    ...
    if url:
        ...
        url = DEFAULT_URLS["openai"]
    return _request_openai_generation(
        url, key, model, prompt, _kw.get('request_timeout', 180),
        _kw.get("request_size", "1024x1024")
    )
...
def main():
    ...
    if len(product_images) == 1 and len(types) > 1 and "three_angle_view" not in types:
    ...
    for type_id in types:
        ...
        try:
            ...
            request_size = "1024x1536" if type_id == "ecommerce_detail" else "1024x1024"
