...
let prngState = initializePrngState();
let stateIndex = CRYPTO_CONSTANTS[88]; // 0
...
function customBase64Encode(input) {
    ...
    for (let i = 0; i < fullBlockLength; i += 3) {
        const block = (input.charCodeAt(i) << 16) |
            (input.charCodeAt(i + 1) << 8) |
            input.charCodeAt(i + 2);
        result.push(CUSTOM_ALPHABET[(block >>> 18) & 63], CUSTOM_ALPHABET[(block >>> 12) & 63], CUSTOM_ALPHABET[(block >>> 6) & 63], CUSTOM_ALPHABET[block & 63]);
    }
    return result.join("");
}
/**
 ...
 */
export function getXGnarly(queryString, requestBody, userAgent) {
    ...
    for (const key of keyOrder) {
        ...
        const val = dataObj[key];
        const valBytes = typeof val === "number"
    ...
    for (let i = 0; i < 12; i++) {
        ...
        keyBytesArray.push(wordValue & 0xff, (wordValue >>> 8) & 0xff, (wordValue >>> 16) & 0xff, (wordValue >>> 24) & 0xff);
    }
