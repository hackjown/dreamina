...
import { RATIO_OPTIONS, DURATION_OPTIONS, REFERENCE_MODES, MODEL_OPTIONS, PROVIDER_OPTIONS, normalizeModelId } from '../types/index';
...
export default function SingleTaskPage() {
  const { state } = useApp();
  const provider = ((state.settings.provider === 'manual-dreamina' ? 'dreamina' : state.settings.provider) || 'dreamina') as ProviderId;
  const availableModels = MODEL_OPTIONS.filter((option) => option.provider === provider);
  const [images, setImages] = useState<UploadedImage[]>([]);
  const [prompt, setPrompt] = useState('');
  const [model, setModel] = useState<ModelId>(normalizeModelId(state.settings.model));
  ...
  const isImageMode = selectedModelOption?.category === 'image';

  useEffect(() => {
    if (!availableModels.some((option) => option.value === model)) {
      setModel((availableModels[0]?.value || 'seedance-2.0-fast') as ModelId);
    }
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (result.data && result.data.length > 0 && result.data[0].url) {
        ...
        setGeneration({
          status: 'error',
          error: `未获取到${isImageMode ? '图片' : '视频'}结果，请重试`,
        });
      }
    } catch (error) {
      setGeneration({
    ...
    <div className="h-screen flex flex-col md:flex-row overflow-hidden bg-[#0f111a] text-white">
      ...
      <div className="flex-1 md:w-[520px] md:flex-none md:border-r border-gray-800 overflow-y-auto custom-scrollbar p-4 md:p-6 bg-[#0f111a]">
        ...
        <div className="hidden md:flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold">{selectedModelOption?.label || (isImageMode ? '图片配置' : '视频配置')}</h2>
          <button
            onClick={() => navigate('/settings')}
            className="p-2 rounded-lg hover:bg-gray-800 transition-colors"
            title="设置"
        ...
        <div className="space-y-5">
          ...
          <div className="bg-[#1c1f2e] rounded-2xl p-4 border border-gray-800">
            ...
            <textarea
              ...
              placeholder={isImageMode ? "描述你想要生成的图片场景。上传多张参考图后可用 @1、@2 或 Image1、Image2 精确引用。" : "描述你想要生成的视频场景。上传参考图后可使用 @1、@2 等引用图片，例如：@1 作为首帧，@4 作为尾帧，模仿 @2 @3 的动作..."}
              value={prompt}
          ...
          <div className="bg-[#1c1f2e] rounded-2xl p-4 border border-gray-800 space-y-5">
            <div>
              ...
              <div className="px-4 py-3 rounded-lg border border-gray-700 bg-[#161824]">
                <div className="text-sm font-medium text-gray-200">
                  {PROVIDER_OPTIONS.find((option) => option.value === provider)?.label || provider}
                </div>
                <div className="text-xs text-gray-500 mt-0.5">
                  {PROVIDER_OPTIONS.find((option) => option.value === provider)?.description || '请前往设置页切换 provider'}
                </div>
              </div>
            </div>
            ...
            {/* Duration - Only shown for video models */}
            {!isImageMode && (
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-3">
                  视频时长
